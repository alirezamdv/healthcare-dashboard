<template>
  ...
  >
    ...
    <slot v-bind="slotData">
      ...
      >
        <option
          v-bind:value="daytime.time"
          v-for="daytime in daytimes"
          v-bind:key="daytime.time"
        >
          {{ daytime.time }}
        </option>
      </select>
...
export default {
  inheritAttrs: false,
  name: "base-select-daytime",
  props: {
    required: {
      type: Boolean,
      description: "Whether select daytime is required (adds an asterisk *)",
    },
    valid: {
      type: Boolean,
